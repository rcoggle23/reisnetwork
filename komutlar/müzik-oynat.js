...
.setDescription(`<a:yan:1096417554710417518>  **| Müzik oynatabilmek için bir ses kanalında olmanız gerekmektedir!**`)
...
.setDescription(`<a:yan:1096417554710417518>  **| Müzik oynatabilmek için aranacak kelime veyahut kelimeler giriniz!**`)
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());    
...
const sarki = await client.player.addToQueue(message.guild.id, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("Reis Network | Kuyruğa Ekle")
  .setDescription(`<a:yan:1096417554710417518> **|** \`${sarki.name}\` **adlı müzik,**\n${message.author} **tarafından kuyruğa eklendi!**`)
.setThumbnail(`https://drummofficial.com/wp-content/uploads/2017/08/equalizer3.gif`)
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());   
...
const sarki = await client.player.play(message.member.voice.channel, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("Reis Network | Oynatılan Şarkı")
  .setDescription(`<a:yan:1096417554710417518> **|** \`${sarki.name}\` **adlı müzik şu anda oynatılıyor.**`)
...
sarki.queue.on('end', () => {
  ...
  .setColor(AloneDogru)
  .setTitle("Reis Network | Kuyruk Bitti!")
  .setDescription(`<a:yan:1096417554710417518> **| Kuyruktaki tüm müzikler oynatıldı. Reis Network kanaldan ayrılıyor.\n\nReis Network Tercih ettiğiniz için teşekkür ederiz.**`)
  .setThumbnail(client.user.avatarURL())
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());  
