...
let logkanal = await db.fetch(`log_${message.guild.id}`)
  ...
  if (args[0] === "sıfırla" || args[0] === "kapat") {
    if(!logkanal) return message.channel.send(`<a:yan:1096417554710417518> **Modlog Kanalı Zaten ayarlı değil**`);
    db.delete(`log_${message.guild.id}`)
   message.channel.send(`<a:yan:1096417554710417518> **ModLog Kanalı başarıyla sıfırlandı.**`);
  
    return
  }
  
if (!logk) return message.channel.send(`<a:yan:1096417554710417518> **Bir modlog kanalı belirtmelisin.**`);
